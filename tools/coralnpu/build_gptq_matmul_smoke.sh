...
EXPERT_FIRMWARE_TARGET="//hw_sim:gem5_gptq_expert.elf"
...
"${BAZEL}" --output_user_root="${BAZEL_OUTPUT_ROOT}" build \
    ...
    "${PROJECTION_FIRMWARE_TARGET}" "${EXPERT_FIRMWARE_TARGET}" "$@"
...
EXPERT_FIRMWARE="$(resolve_output "${EXPERT_FIRMWARE_TARGET}" "$@")"
[ -f "${BRIDGE}" ] || {
...
[ -f "${EXPERT_FIRMWARE}" ] || {
    echo "error: GPTQ expert firmware output not found: ${EXPERT_FIRMWARE}" >&2
    exit 1
}
...
install_output "${EXPERT_FIRMWARE}" \
    "${OUT_DIR}/gem5_gptq_expert.elf" 0644
...
echo "built: ${OUT_DIR}/gem5_gptq_expert.elf"
