...
class SCore(p: Parameters) extends Module {
  ...
  dispatch.io.xnpuReject := io.xnpu.reject
  ...
  // Coral performs only CUSTOM_3 classification. The NPU side owns secondary
  // decode and accepts the durable command before this instruction retires.
  io.xnpu.request.valid := dispatch.io.xnpu.valid
  dispatch.io.xnpu.ready := io.xnpu.request.ready
  io.xnpu.request.bits.instruction := dispatch.io.xnpu.bits.instruction
  io.xnpu.request.bits.pc := dispatch.io.xnpu.bits.pc
  io.xnpu.request.bits.rs1Value := regfile.io.xnpuReadData(0)
  io.xnpu.request.bits.rs2Value := regfile.io.xnpuReadData(1)
  io.xnpu.request.bits.rdValue := regfile.io.xnpuReadData(2)
  io.xnpu.request.bits.csr := csr.io.xnpu
  regfile.io.xnpuReadAddr(0) := dispatch.io.xnpu.bits.rs1
  regfile.io.xnpuReadAddr(1) := dispatch.io.xnpu.bits.rs2
  regfile.io.xnpuReadAddr(2) := dispatch.io.xnpu.bits.rd
  assert(!(io.xnpu.request.ready && io.xnpu.reject))
  when (io.xnpu.request.fire) {
    assert(io.xnpu.request.bits.instruction(6, 0) ===
        XOpenNpuEncoding.Custom3Opcode)
  }
