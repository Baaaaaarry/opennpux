...
if [ "$HOST_FUNCTIONAL_EXECUTION_MODE" = xopennpux-primitives ]; then
    ...
    HOST_XGRAPH_REQUIRE_FULL="${CORAL_HOST_XGRAPH_REQUIRE_FULL:-1}"
fi
[ "$#" -eq 0 ] || {
...
GPTQ_EXPERT_ACCUMULATION="${CORAL_GPTQ_EXPERT_ACCUMULATION:-$GPTQ_ACCUMULATION}"
LINEAR_ATTENTION_PREFILL="${CORAL_LINEAR_ATTENTION_PREFILL:-recurrent}"
EXPERT_INTERMEDIATE_PRECISION="${CORAL_GPTQ_EXPERT_INTERMEDIATE_PRECISION:-fp32}"
case "$GPTQ_ACCUMULATION:$GPTQ_EXPERT_ACCUMULATION" in
    fp32:*|fp64:*|grouped:*|wna16:*|mma16:*|mma16-bf16:*) ;;
    *)
        echo "error: unsupported CORAL_GPTQ_ACCUMULATION mode: $GPTQ_ACCUMULATION" >&2
        exit 2
        ;;
esac
case "$GPTQ_EXPERT_ACCUMULATION" in
    fp32|fp64|grouped|wna16|mma16|mma16-bf16) ;;
    *)
        echo "error: unsupported CORAL_GPTQ_EXPERT_ACCUMULATION mode: $GPTQ_EXPERT_ACCUMULATION" >&2
        exit 2
        ;;
esac
case "$LINEAR_ATTENTION_PREFILL" in
    recurrent|chunk) ;;
    *)
        echo "error: CORAL_LINEAR_ATTENTION_PREFILL must be recurrent or chunk" >&2
        exit 2
        ;;
esac
case "$EXPERT_INTERMEDIATE_PRECISION" in
    fp32|bf16) ;;
    *)
        echo "error: CORAL_GPTQ_EXPERT_INTERMEDIATE_PRECISION must be fp32 or bf16" >&2
        exit 2
        ;;
esac
...
echo "[coral-qwen35b-real-weights-test] GPTQ expert accumulation: $GPTQ_EXPERT_ACCUMULATION" >&2
echo "[coral-qwen35b-real-weights-test] linear attention prefill: $LINEAR_ATTENTION_PREFILL" >&2
...
if [ "$SIM_HOST_FUNCTIONAL" != 0 ]; then
    ...
    if [ "${CORAL_SKIP_HOST_FUNCTIONAL_PREFLIGHT:-0}" = 0 ]; then
        ...
        OPENNPUX_HOST_FUNCTIONAL_EXECUTION="$HOST_FUNCTIONAL_EXECUTION_MODE" \
        OPENNPUX_HOST_XGRAPH_DEBUG="${CORAL_HOST_XGRAPH_DEBUG:-0}" \
        OPENNPUX_HOST_XGRAPH_SHADOW_COMPARE="${CORAL_HOST_XGRAPH_SHADOW_COMPARE:-0}" \
        OPENNPUX_HOST_XGRAPH_OPCODE_MASK="${CORAL_HOST_XGRAPH_OPCODE_MASK:-0xffffffff}" \
        OPENNPUX_HOST_XGRAPH_GPTQ_MATMUL_SCOPE="$HOST_XGRAPH_GPTQ_MATMUL_SCOPE" \
        OPENNPUX_HOST_XGRAPH_NORMALIZE_SCOPE="$HOST_XGRAPH_NORMALIZE_SCOPE" \
        OPENNPUX_HOST_XGRAPH_REQUIRE_FULL="$HOST_XGRAPH_REQUIRE_FULL" \
        ...
        OPENNPUX_GPTQ_EXPERT_INTERMEDIATE_PRECISION="$EXPERT_INTERMEDIATE_PRECISION" \
...
set -u
BB=/tmp/busybox
if [ ! -x "\$BB" ] || [ ! -f /tmp/coralnpu-baseline-preload.ready ]; then
    echo '[coral-qwen35b-real-weights-test] FAIL: stale checkpoint preload'
    m5 --inst exit
    exit 1
fi
