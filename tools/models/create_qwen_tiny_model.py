...
def build_package() -> dict:
    ...
    return {
        ...
        "numeric_reference": {
            "numeric_reference_version": 1,
            "epsilon": epsilon,
            "runtime_input_ids": input_ids,
            "token_embedding": flatten(token_embedding),
            "lm_head": flatten(lm_head),
            "rms_attn_weight": rms_attn_weight,
            "rms_ffn_weight": rms_ffn_weight,
            "wq": flatten(wq),
            "wk": flatten(wk),
            "wv": flatten(wv),
            "wo": flatten(wo),
            "w_gate": flatten(w_gate),
            "w_up": flatten(w_up),
            "w_down": flatten(w_down),
        },
        "golden": {
