#!/usr/bin/env python3
"""Create a Relax Transformer block with persistent recurrent decode state."""
...
def build_module():
    ...
    recurrent_state = relax.Var(
        "recurrent_state", relax.TensorStructInfo([2, 64], "float32")
    )
    ...
    with builder.function(
        "main",
        [hidden, recurrent_state, norm_weight, projection_weight, residual],
    ):
        with builder.dataflow():
            ...
            state_mixed = builder.emit(
                relax.op.add(projected, recurrent_state),
                name_hint="state_mixed",
            )
            residual_sum = builder.emit(
                relax.op.add(state_mixed, residual), name_hint="residual_sum"
            )
...
def main() -> None:
    ...
    print("tvm_stateful_transformer_block=PASS")
