...
uint32_t opennpux_npu_route_table_checksum(const void *buffer, size_t size);
int opennpux_npu_route_table_build(
    const struct opennpux_npu_route_record *records, uint32_t record_count,
    void *buffer, size_t capacity, size_t *table_size);
int opennpux_npu_route_table_validate(const void *buffer, size_t size);
