...
tsoftmax_count = sum((word & 0xFE00707F) == 0x6400207B for word in words)
trope_count = sum((word & 0xFE00707F) == 0x6600207B for word in words)
...
checks = {
    ...
    "tsoftmax_encoding_count": (tsoftmax_count, 1),
    "trope_encoding_count": (trope_count, 1),
