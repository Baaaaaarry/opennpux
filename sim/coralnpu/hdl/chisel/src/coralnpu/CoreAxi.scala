...
class CoreAxi(p: Parameters, coreModuleName: String) extends RawModule {
  ...
  val io = IO(new Bundle {
    ...
    // OpenNPUX coprocessor command interface. The external NPU performs L2
    // decode and returns either durable acceptance or precise rejection.
  ...
  withClockAndReset(rst_sync.io.clk_o, global_reset) {
    ...
    io.xnpu.request.valid := core.io.xnpu.request.valid
    io.xnpu.request.bits := core.io.xnpu.request.bits
    core.io.xnpu.request.ready := io.xnpu.request.ready
    core.io.xnpu.reject := io.xnpu.reject
