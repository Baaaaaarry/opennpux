...
class CoreMiniAxiWrapper {
 public:
  ...
  void RegisterDeferredReadCallback(
    ...
    master_read_driver_.RegisterDeferredReadCallback(read_cb);
  }

  void RegisterDeferredWriteCallback(
      std::function<void(const AxiAddr&, const AxiWData&)> write_cb) {
    master_write_driver_.RegisterDeferredWriteCallback(write_cb);
  }

  void QueueReadResponse(const AxiRData& response) {
    master_read_driver_.QueueReadResponse(response);
  }

  void QueueWriteResponse(const AxiWResp& response) {
    master_write_driver_.QueueWriteResponse(response);
  }

  bool HasDeferredRequest() const {
    return master_read_driver_.HasDeferredRequest() ||
  ...
  bool WaitForTermination(int timeout = 10000) {
    for (int i = 0; i < timeout; i++) {
      if ((*halted_) || (*wfi_) || HasDeferredRequest()) {
        return true;
      }
      Step();
    }
