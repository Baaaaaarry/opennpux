...
class XOpenNpuCsrState(p: Parameters) extends Bundle {
  ...
  val tensorFeatures = UInt(p.xlen.W)
...
class DecodedInstruction(p: Parameters) extends Bundle {
  ...
  val sh    = Bool()
  val sw    = Bool()
  // OpenNPUX custom-0 command submission instruction. The instruction uses
  // rs1 as the operator MMIO base and rs2 as the descriptor address. Dispatch
  ...
  def isScalarStore(): Bool = { sb || sh || sw || npuLaunch }
...
class DispatchV2(p: Parameters) extends Dispatch(p) {
  ...
  for (i <- 0 until p.instructionLanes) {
    ...
    val lsu = SafeMuxUpTo1H(MakeValid(false.B, LsuOp.LB), Seq(
      ...
      (d.sw || d.npuLaunch) -> MakeValid(true.B, LsuOp.SW),
    ...
    io.lsu(i).bits.store := io.inst(i).bits.inst(5) || d.npuLaunch
  ...
  for (i <- 0 until p.instructionLanes) {
    ...
    io.busRead(i).immed := MuxCase(
        Cat(d.imm12(31,5), Mux(storeSelect, d.immst(4,0), d.imm12(4,0))),
        IndexedSeq(
          d.rvv.map(_.valid).getOrElse(false.B) -> 0.U,
          d.npuLaunch -> 4.U))
  }
}
...
object DecodeInstruction {
  def apply(p: Parameters, pipeline: Int, addr: UInt, op: UInt,
    ...
    // CUSTOM_0, funct7=0, funct3=0, rd=x0.
