...
int main() {
  auto* request = reinterpret_cast<volatile coral_gptq_expert_request*>(
      static_cast<uintptr_t>(kExtmemBase));
  ...
  opennpux::InitializeOperatorDescriptor(
      descriptor, CORAL_OPERATOR_OP_GPTQ_GATED_MLP,
  ...
  if (!opennpux::SetOperatorTensor(
          descriptor, 0, kExtmemBase, sizeof(*request), 1, dimensions,
          CORAL_OPERATOR_ELEMENT_INT8, 0) ||
      !opennpux::SubmitHybridOperator(descriptor, kDescriptorAddress)) {
    return 1;
  }
  return request->state == CORAL_GPTQ_EXPERT_COMPLETE &&
