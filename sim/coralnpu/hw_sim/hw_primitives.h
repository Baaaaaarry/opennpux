...
class AxiMasterReadDriver : Clock::Observer {
 public:
  ...
  bool HasDeferredRequest() const { return deferred_request_pending_; }

 private:
  void OnFallingEdge() final {
    *read_addr_ready_ = !deferred_request_pending_;
    ...
    *read_data_valid_ = !data_queue_.empty();
    ...
    if (*read_addr_valid_ && *read_addr_ready_) {
      ...
      if (deferred_read_cb_) {
        deferred_request_pending_ = true;
        *read_addr_ready_ = 0;
        deferred_read_cb_(axi_addr_);
      } else if (read_cb_) {
 ...
 private:
  void OnFallingEdge() final {
    ...
    *write_resp_valid_ = !resp_queue_.empty();
    ...
    // Receive Addr
    if (*write_addr_valid_ && !deferred_request_pending_) {
    ...
    // Receive Data
    if (*write_data_valid_ && !deferred_request_pending_) {
