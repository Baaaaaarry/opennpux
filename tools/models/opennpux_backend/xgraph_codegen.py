"""Lower frontend-neutral OpenNPUX graph IR to the XGraph v2 ABI."""
...
def _fuse_transposed_rhs(nodes: Any) -> tuple[list[dict[str, Any]], int]:
    ...
    skipped: set[int] = set()
    for node_index, node in enumerate(nodes):
        if normalize_operation(node.get("op")) != "transpose":
            continue
...
class CodegenError(ValueError):
    """The backend graph cannot be represented by the current XGraph ABI."""
...
def compile_graph(
    ...
    """Compile normalized backend graph IR to binary and inspectable metadata."""
