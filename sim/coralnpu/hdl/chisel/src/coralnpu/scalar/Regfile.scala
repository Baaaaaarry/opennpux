...
class Regfile(p: Parameters) extends Module {
  ...
  val io = IO(new Bundle {
    ...
    // The tensor instruction encodes three address operands in rs1/rs2/rd.
    // Dedicated combinational reads avoid repurposing the scalar execute ports.
    val xnpuReadAddr = Input(Vec(3, UInt(log2Ceil(p.scalarRegCount).W)))
    val xnpuReadData = Output(Vec(3, UInt(p.xlen.W)))
  ...
  for (i <- 0 until 3) {
    val idx = io.xnpuReadAddr(i)
    io.xnpuReadData(i) := Mux(VecAt(writeValid, idx),
                                 VecAt(writeData, idx), VecAt(regfile, idx))
  }
