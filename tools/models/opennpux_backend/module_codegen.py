"""Compile a DAG of OpenNPUX backend regions into reusable XGraph artifacts."""
...
from opennpux_backend.canonicalize import canonicalize_module
...
def compile_module(
    ...
    for index, record in enumerate(raw_regions):
        ...
        if not isinstance(graph, dict) or graph.get("format") not in GRAPH_FORMATS:
            raise CodegenError(f"region {name} must contain normalized backend graph IR")
        graphs[name] = graph
